...
The account registry and native Codex profile routing are implemented. Next validate real
multi-account execution, extend provider support and shared account recovery, and continue CI
and repeat-run measurement.
...
| 3a | **Configure providers, accounts, and agent teams.** Named Codex connections now support credential provisioning, project grants, isolated runtime selection, pinned profile assignments and attempt attribution. Next validate real multi-account work, then add a tested Claude adapter and shared account scheduling. | Two accounts of the same provider run without credential/session crossover; a mixed Codex/Claude team records actual task attribution. Revocation, reconnect, capacity and explicit fallback are tested. Secrets stay out of workflow JSON and snapshots. See [delivery slices](AGENT_ACCOUNTS.md). |
